...
class TestSwiSignature( unittest.TestCase ):
    ...
    def _verifySignature( self, filename ):
        with tempfile.TemporaryDirectory( prefix="swix-verify-" ) as workDir:
            retCode = verify.verifyAllSwi( workDir, filename, self.root_crt )
            self.assertEqual( retCode, 0 )

    def test_prepare_return_hexdigest( self ):
        hexdigest = signature.prepareSwi( self.test_swi )
    ...
    def test_prepare_already_signed( self ):
        ...
        with self.assertRaises( signature.SwiSignException ):
            signature.prepareSwi( self.test_swi )

    def test_prepare_already_signed_force( self ):
        with zipfile.ZipFile( self.test_swi, 'a' ) as swi:
            swi.writestr( SIG_FILE_NAME, 'random signature' )
        signature.prepareSwi( self.test_swi, forceSign=True )
        self._validateNullSig( self.test_swi )

    def test_sign_without_prepare( self ):
        with self.assertRaises( signature.SwiSignException ):
            signature.signSwi( self.test_swi, self.signing_crt, self.root_crt )
    ...
    def test_sign_with_signature( self ):
        signature.prepareSwi( self.test_swi )
        mock_signature = MockSigningServer.getTestSignature( self.test_swi )
    ...
    def test_sign_and_verify_with_wrong_root_crt( self ):
        signature.prepareSwi( self.test_swi )
        with self.assertRaises( signature.SwiSignException ):
            signature.signSwi( self.test_swi, self.signing_crt, self.signing_crt,
                                  signingKeyFile=self.signing_key )
    ...
    def test_signature_bigger_than_null_sig( self ):
        ...
        with self.assertRaises( signature.SwiSignException ):
            signature.signSwi( self.test_swi, bigCertFile, self.root_crt,
                                  signingKeyFile=self.signing_key )
